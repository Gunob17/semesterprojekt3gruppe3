///by Sarah Trohl, Simon Askholm, Frederik Hansen and Gustav Nobel
...
void MainWindow::on_loadButton_clicked() {
	...
	query->prepare("SELECT * FROM test123.opencv;");
	query->exec();
...
void MainWindow::on_startButton_clicked() {
	...
	QString toSQL = "INSERT INTO opencv VALUES (NULL," + x + ", " + y + ",NULL,NULL,NULL,NULL)";
...
void MainWindow::on_Xinput_valueChanged() {
	...
	if (y_spin <= 12.4321*x_spin + (-4497.037)) {
		ui->label5->setText("Y-axis has to be between: " + QString::number((int)(0.9815498*(x_spin)+(-1130.576))) +
			" and: " + QString::number((int)(-1.030369*x_spin + 551.3884)));
		ui->Yinput->setRange(0.9815498*(x_spin)+(-1130.576), -1.030369*x_spin + 551.3884);
	}
	else if (y_spin > 12.4321*x_spin + (-4497.037)) {
		ui->label5->setText("Y-axis has to be between: " + QString::number((int)(-1.030905*x_spin + -538.9139)) + 
			" and: " + QString::number((int)(0.9775281*x_spin + -201.573)));
...
void MainWindow::on_throwButton_clicked() {
	...
	QString toSQL = "UPDATE `test123`.`opencv` SET `throwx` = " + QString::number(point.x) + ", `throwy` = " + QString::number(point.y) + " WHERE (`testID` = " + QString::number(place) + ");";
...
void MainWindow::on_verifyButton_clicked() {
	...
	QSqlQuery* query2 = new QSqlQuery(conn.db);
	QString toSQL = "UPDATE `test123`.`opencv` SET `landx` = " + x + ", `landy` = " + y + " WHERE(`testID` = " + QString::number(place) + ");";
	query2->prepare(toSQL);
	query2->exec();
