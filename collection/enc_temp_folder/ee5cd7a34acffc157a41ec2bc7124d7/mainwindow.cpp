...
void MainWindow::on_Xinput_valueChanged(int arg1) {
	...
	x_spin = ui->Xinput->value();
	y_spin = ui->Yinput->value();
	if (y_spin <= 0.9815498*(x_spin + 300) + (-525.0406)) {
		ui->Yinput->setValue(0.9815498*x_spin + (-525.0406));
		ui->label->setText("y axis has to be between: " + QString::number(-1.030369*x_spin + 1760.499) + " and: " + QString::number(0.9815498*x_spin + (-525.0406)));
	}
	if (y_spin <= -1.030369*(x_spin + 300) + 1760.499) {
		ui->Yinput->setValue(-1.030369*x_spin + 1760.499);
	}
	if (y_spin <= 0.9775281*(x_spin + 300) + 405.1685) {
		ui->Yinput->setValue(0.9775281*x_spin + 405.1685);
	}
	if (y_spin <= -1.030905*(x_spin + 300) + 670.3576) {
		ui->Yinput->setValue(-1.030905*x_spin + 670.3576);
	}
